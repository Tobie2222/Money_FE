...
import { deleteTran, FilterTransaction, findTransaction, getAllCatIncome, getAllTranIncome, updateTran } from '../../data/Api'
...
export default function TranIncome() {
    ...
    const formatCategories = (categories) => {
        return categories.map((cat) => {
            return {
                id: cat._id,
                value: cat.income_type_name.length > 6 ? `${cat.income_type_name.substring(0, 6)}...` : cat.income_type_name,
    ...
    const handleUpdateTran = async () => {
        ...
        try {
            ...
            const response = await updateTran("income",selectId, user?.id, payload, {
                headers: {
                    token: `Bearer ${token}`
                }
            })
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const response = await findTransaction(useDebounceSearchItem, user?.id, "income", {
                headers: {
                    token: `bearer ${token}`
                }
            })
    ...
    const handleFilter = async (type = "") => {
        ...
        try {
            ...
            // Gọi API lọc
            const response = await FilterTransaction("income", filterDate, filterCategories, user?.id, {
                headers: {
                    token: `bearer ${token}`
                }
            })
            if (response.status === 200) {
                setTranIncome(response.data.transactions)
            }
        } catch (err) {
            console.log(err)
        } finally {
            ...
            >
                <View className="flex-1 justify-center items-center">
                    ...
                    <View className="w-[300px] p-[20px] bg-white rounded-[18px] z-10">
                        <Text className="text-center font-[600] text-[16px] text-textColor ">Bạn có muốn sửa khoản thu này</Text>
                        <View className="flex flex-col mt-[20px] ">
                            <TextInput
                                placeholder='Tên danh khoản thu '
